...
local HeroEquipProxy = class("HeroEquipProxy", RemoteProxy)
HeroEquipProxy.NAME = global.ProxyTable.HeroEquipProxy

function HeroEquipProxy:ctor()
    HeroEquipProxy.super.ctor(self)
    ...
    self.ItemBelongType = global.MMO.ITEM_FROM_BELONG_HEROEQUIP
    self.posfight = {}
    self.suitnum = {}
    self.lastTakeOnPos = {}
    ...
    self._helmet_cap_part = nil --头盔斗笠内观是否是分开显示的

    self:loadJson()
end

function HeroEquipProxy:loadJson()
...
function HeroEquipProxy:loginOut()
    return self._helmet_cap_part
end

function HeroEquipProxy:GetEquipModelOffSet()
    return self.ModelViewEquipOff
end

function HeroEquipProxy:GetModelHairOffSet()
    return self.ModelViewHairOff
end

function HeroEquipProxy:GetEquipPosByStdMode(StdMode)
...
-- tips部位列表
function HeroEquipProxy:GetTipsEquipPosByStdModeList(StdMode)
...
function HeroEquipProxy:GetEquipPosByStdModeList(StdMode)
    return self.EquipPosData.EquipPosByStdMode[StdMode]
end

function HeroEquipProxy:GetEquipMappingConfig(pos)
    return self.EquipPosData.EquipPosMapping[pos]
end
-- 反向
function HeroEquipProxy:GetDeEquipMappingConfig(pos)
    if not pos then
        return nil
    end
...
function HeroEquipProxy:IsShowEquipItems(pos)
...
function HeroEquipProxy:GetEquipTypeConfig()
    return self.EquipPosType
end

function HeroEquipProxy:ClearEquipData()
    self._equipDataByMakeIndex = {}
    self._equipPosData = {}
end

function HeroEquipProxy:GetEquipData()
    return self._equipDataByMakeIndex
end

function HeroEquipProxy:GetEquipPosData()
    return self._equipPosData
end

function HeroEquipProxy:GetEquipDataByMakeIndex(makeIndex)
    return self._equipDataByMakeIndex[makeIndex]
end

function HeroEquipProxy:FindEquipDataByPos(pos)
    if not self._equipPosData[pos] then
        return nil
    end
    return self:GetEquipDataByMakeIndex(self._equipPosData[pos])
end
function HeroEquipProxy:GetEquipDataByPos(pos, beginOnMoving)
...
function HeroEquipProxy:GetEquipDataByName(Name)
    local list = {}
...
function HeroEquipProxy:GetEquipDataPosDataList(pos)
    local data = {}
    local list = self:GetEquipMappingConfig(pos) -- 单显示位置 多个装备位置共享
    if list then
        for _, v in ipairs(list) do
...
function HeroEquipProxy:GetMinPowerPosByStdMode(StdMode, param, checkPosData)
    ...
    local isChekcPos = false
    if checkPosData then
        isChekcPos = true
    end

    for k, v in ipairs(pos) do
        local equipData = isChekcPos and v.data or self:GetEquipDataByPos(v)
        if not equipData then
            minPowerPos = isChekcPos and v.pos or v
            onEquipMinPower = 0
            hasEquip = false
            break
        end
        local equipPower = GUIFunction:GetEquipPower(equipData, param, true)
...
function HeroEquipProxy:GetEquipTakeOnPosByStdMode(StdMode, param)
    local StdMode = StdMode or 0
    local onEquipMinPower = 0
    local minPowerPos = -1
    local hasEquip = true
...
function HeroEquipProxy:CheckEquipNeedSex(item, sex)
    if not item or not next(item) then
        return false
    end
    local PlayerPropertyProxy = global.Facade:retrieveProxy(global.ProxyTable.HeroPropertyProxy)
...
function HeroEquipProxy:CheckEquipWeight(onItem, pos)
...
function HeroEquipProxy:CheckBujukUse(item, putPos)
    -- body
...
function HeroEquipProxy:CheckBestRingsSpace()
...
function HeroEquipProxy:GetBestRingsOpenState()
    return self.bestRingsOpen
end

function HeroEquipProxy:SetBestRingsOpenState(state)
    self.bestRingsOpen = state
end

function HeroEquipProxy:AddEquipData(item, isInit)
    ...
    if not isInit then
        ...
        sData.ItemData = item
        global.Facade:sendNotification(global.NoticeTable.PlayEquip_Oper_Data_Hero, sData)

        SLBridge:onLUAEvent(LUA_EVENT_HERO_EQUIP_CHANGE, sData)
    end
end

function HeroEquipProxy:DelEquipData(item)
    local MakeIndex = item.MakeIndex
...
function HeroEquipProxy:ChangeEquipData(item, bagDelayUpdate, isChangeLook)
    local MakeIndex = item.MakeIndex
    local Where = item.Where
...
function HeroEquipProxy:SetEmbattle(data)
    self.embattle = {}
...
function HeroEquipProxy:GetEmbattle()
    return self.embattle
end

function HeroEquipProxy:handle_MSG_SC_EQUIP_Embattle_RESPONSE(msg)
    local header = msg:GetHeader()
    local recog = header.recog
    self:SetEmbattle(recog)
    global.Facade:sendNotification(global.NoticeTable.Layer_Embattle_Refresh_Hero)
    SLBridge:onLUAEvent(LUA_EVENT_HERO_EMBATTLE_CHANGE)
end

-- --更换iconID
function HeroEquipProxy:ChangeEquipLook(MakeIndex)
...
-- 自定义装备使用
function HeroEquipProxy:SetCustUserItem(custUserItem)
    ...
    ItemsConfig.EquipMapByStdMode = ItemsConfig.EquipMapByStdModeTemp

    self.EquipPosData.EquipPosByStdMode = {}
    self.EquipPosData.EquipPosByStdMode = self.EquipPosData.EquipPosByStdModeTemp
...
function HeroEquipProxy:handle_MSG_SC_PLAYER_EQUIP_ON_FAIL(msg)
    local header = msg:GetHeader()
    local data = {}
    data.isSuccess = false
    data.header = header
    global.Facade:sendNotification(global.NoticeTable.HeroTakeOnResponse, data)
end

function HeroEquipProxy:handle_MSG_SC_PLAYER_EQUIP_ON_SUCCESS(msg)
    local header = msg:GetHeader()
    local data = {}
    data.isSuccess = true
    data.header = header
...
function HeroEquipProxy:handle_MSG_SC_PLAYER_EQUIP_OFF_SUCCESS( msg )
    local header = msg:GetHeader()
    global.Facade:sendNotification(global.NoticeTable.HeroTakeOffResponse, {isSuccess = true, pos = header.recog})
end

function HeroEquipProxy:handle_MSG_SC_PLAYER_EQUIP_OFF_FAIL(msg)
    global.Facade:sendNotification(global.NoticeTable.HeroTakeOffResponse, {isSuccess = false, header = msg:GetHeader()})
end

function HeroEquipProxy:handle_MSG_SC_PALYER_EQUIP_INFO_HERO(msg)
    ...
    global.Facade:sendNotification(global.NoticeTable.PlayEquip_Oper_Init_Hero)
    SLBridge:onLUAEvent(LUA_EVENT_HERO_EQUIP_CHANGE, {opera = global.MMO.Operator_Init})
end

function HeroEquipProxy:RequestBestRingsOpenState()
    
    LuaSendMsg(global.MsgType.MSG_CS_PLAYER_EQUIP_BEST_RINGS_STATE_HERO, 0, 0, 0, 0)
end

function HeroEquipProxy:handle_MSG_SC_PLAYER_EQUIP_BEST_RINGS_STATE(msg)
    local header = msg:GetHeader()
    ...
    global.Facade:sendNotification(global.NoticeTable.Layer_PlayerBestRing_State_Hero, { isOpen = isOpen })
    SLBridge:onLUAEvent(LUA_EVENT_HERO_BESTRINGBOX_STATE, {isOpen = isOpen})
end

function HeroEquipProxy:onRegister()
    HeroEquipProxy.super.onRegister(self)
end

function HeroEquipProxy:RegisterMsgHandler()
    HeroEquipProxy.super.RegisterMsgHandler(self)
    local msgType = global.MsgType

    --初始化装备信息
    LuaRegisterMsgHandler(msgType.MSG_SC_PALYER_EQUIP_INFO_HERO, handler(self, self.handle_MSG_SC_PALYER_EQUIP_INFO_HERO))
    -- 装备穿上确认-成功
    LuaRegisterMsgHandler(msgType.MSG_SC_PLAYER_EQUIP_ON_SUCCESS_HERO, handler(self, self.handle_MSG_SC_PLAYER_EQUIP_ON_SUCCESS))
    -- -- 装备穿上确认-失败
    LuaRegisterMsgHandler(msgType.MSG_SC_PLAYER_EQUIP_ON_FAIL_HERO, handler(self, self.handle_MSG_SC_PLAYER_EQUIP_ON_FAIL))
    -- -- 装备脱下确认-成功
    LuaRegisterMsgHandler(msgType.MSG_SC_PLAYER_EQUIP_OFF_SUCCESS_HERO, handler(self, self.handle_MSG_SC_PLAYER_EQUIP_OFF_SUCCESS))
    -- -- 装备脱下确认-失败
    LuaRegisterMsgHandler(msgType.MSG_SC_PLAYER_EQUIP_OFF_FAIL_HERO, handler(self, self.handle_MSG_SC_PLAYER_EQUIP_OFF_FAIL))
    -- -- 首饰盒开启状态
    LuaRegisterMsgHandler(msgType.MSG_SC_PLAYER_EQUIP_BEST_RINGS_STATE_HERO, handler(self, self.handle_MSG_SC_PLAYER_EQUIP_BEST_RINGS_STATE))
    -- 法阵
    LuaRegisterMsgHandler( msgType.MSG_SC_EQUIP_Embattle_RESPONSE_HERO,handler(self, self.handle_MSG_SC_EQUIP_Embattle_RESPONSE) )
end

return HeroEquipProxy