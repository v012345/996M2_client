...
function PlayerBestRingLayer:InitGUI(data)
    SL:RequireFile(SLDefine.LUAFile.LUA_FILE_TRADE_PLAYER_BESTRING)
    PlayerBestRing_Look_TradingBank.main(data)
    self._root = PlayerBestRing_Look_TradingBank._ui.Panel_1

    local itemPanelSz = GUI:getContentSize(PlayerBestRing_Look_TradingBank._ui.Panel_touch)
    PlayerBestRing_Look_TradingBank._Panel_width = itemPanelSz.width
    PlayerBestRing_Look_TradingBank._Panel_height = itemPanelSz.height

    GUI:setSwallowTouches(PlayerBestRing_Look_TradingBank._ui.Panel_touch, false)

    self:InitUI()
    self:RefreshEquipList()
    self._CaptureNode = PlayerBestRing_Look_TradingBank._ui.Panel_1
end

function PlayerBestRingLayer:InitHideIconPos()
    self._hideIconPos = {}
    for i, v in ipairs(PlayerBestRing_Look_TradingBank.posSetting or {}) do
...
function PlayerBestRingLayer:InitUI()
    ...
    global.mouseEventController:registerMouseButtonEvent(PlayerBestRing_Look_TradingBank._ui.Panel_touch,
    {
...
function PlayerBestRingLayer:RefreshEquipList()
    local panelItems = PlayerBestRing_Look_TradingBank._ui.Panel_items
    local posSetting = PlayerBestRing_Look_TradingBank.posSetting
    ...
    for index, pos in ipairs(posSetting) do
        ...
        if not self._hideIconPos[pos]  then
            if  PlayerBestRing_Look_TradingBank._HideDefaultIcon and PlayerBestRing_Look_TradingBank._HideDefaultIcon[index] then 
        ...
        if data then
            ...
            goodItem = PlayerBestRing_Look_TradingBank.CreateEquipItemCallBack(goodItem, info)
...
function PlayerBestRingLayer:GetItemBagEmptyPos(touchPos)
    local x = touchPos.x
    local y = touchPos.y
    local panelWorldPos = PlayerBestRing_Look_TradingBank._ui.Panel_touch:getWorldPosition()
    local posXInPanel = x - panelWorldPos.x
    local posYInPanel = panelWorldPos.y - y
    if posXInPanel >= PlayerBestRing_Look_TradingBank._Panel_width or posXInPanel <= 0 then
        return nil
    end

    if posYInPanel >= PlayerBestRing_Look_TradingBank._Panel_height or posYInPanel <= 0 then
        return nil
    end
    ...
    for i, pos in ipairs(PlayerBestRing_Look_TradingBank.posSetting or {}) do
        local equipPanel = panelItems:getChildByName("Node_" .. pos)
        if equipPanel then
            nodeRect.x = equipPanel:getPositionX() - global.MMO.BEST_RING_ITEM_WIDTH / 2
            nodeRect.y = PlayerBestRing_Look_TradingBank._Panel_height - equipPanel:getPositionY() - global.MMO.BEST_RING_ITEM_HEIGHT / 2
...
function PlayerBestRingLayer:RefreshAddTouchSize()
    if PlayerBestRing_Look_TradingBank._ui.Panel_touch and not tolua.isnull(PlayerBestRing_Look_TradingBank._ui.Panel_touch) then
        local itemPanelSz = PlayerBestRing_Look_TradingBank._ui.Panel_touch:getContentSize()
        PlayerBestRing_Look_TradingBank._Panel_width = itemPanelSz.width
        PlayerBestRing_Look_TradingBank._Panel_height = itemPanelSz.height
    end
end
