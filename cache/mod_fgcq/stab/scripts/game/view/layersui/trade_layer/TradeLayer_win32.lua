...
function TradeLayer:InitGUI()
    ...
    self._ui.Button_trade:addClickEventListener(function()
        if not self._proxy:CheckTradeRequest() then
...
function TradeLayer:GetItemBagEmptyPos( touchPos )
    ...
    local indexX = math.ceil(posXInPanel/global.MMO.TRADE_ITEM_SIZE_WIDTH_WIN)
    local indexY = math.floor(posYInPanel/global.MMO.TRADE_ITEM_SIZE_HEIGHT_WIN)
...
function TradeLayer:UpdateItemList( param )
    ...
    local itemHeight = global.MMO.TRADE_ITEM_SIZE_HEIGHT_WIN
    local itemWidth = global.MMO.TRADE_ITEM_SIZE_WIDTH_WIN
    local maxHeight = global.MMO.TRADE_ITEM_PANEL_HEIGHT_WIN
    local maxWidth = global.MMO.TRADE_ITEM_PANEL_WIDTH_WIN
