...
function View:removeObserver(notificationName, notifyContext)
	...
	for i = 1, n do
		local o = observers[i]
		if o and o:compareNotifyContext(notifyContext) then
			table.remove(observers, _)
			break
		end
	end
...
function View:registerMediator(mediator)
	...
	if n > 0 then
		...
		for i = 1, n do
			local name = interests[i]
			if name then
				self:registerObserver(name, observer)
			end
		end
	end
	mediator:onRegister()
...
function View:removeMediator(mediatorName)
	...
	if mediator then
		...
		for i = 1, n do
			local name = interests[i]
			if name then
				self:removeObserver(name, mediator)
			end
		end
