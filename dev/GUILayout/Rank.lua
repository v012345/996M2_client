...
function Rank.main(type)
    ...
    if Rank._rankType == 2 then
        GUI:Image_loadTexture(Rank.UI.ImageView_level, "res/custom/Rank/img_power.png")
    elseif Rank._rankType == 100 then
        GUI:Image_loadTexture(Rank.UI.ImageView_level, "res/custom/Rank/img_power.png")
        Rank.DeploymentGlobalWidget()
    end

    GUI:Button_setBrightEx(Rank.UI["Button_Left_" .. Rank._rankType], false)
    --SL:OpenRankUI(type)
    GUI:addOnClickEvent(Rank.UI.Button_Left_1, function()
        SL:CloseRankUI()
        SL:OpenRankUI(1)
    end)
    GUI:addOnClickEvent(Rank.UI.Button_Left_2, function()
        SL:CloseRankUI()
        SL:OpenRankUI(2)
    end)
    GUI:addOnClickEvent(Rank.UI.Button_Left_100, function()
        SL:CloseRankUI()
        SL:OpenRankUI(100)
    end)
    GUI:addOnClickEvent(Rank.UI.Button_upZhanLi, function()
        ssrMessage:sendmsg(ssrNetMsgCfg.PostRank_Request)
    end)
    Rank.InitTypeBtn()
...
--对自定义全局排行处理控件
function Rank.DeploymentGlobalWidget()
    --隐藏无用的控件
    GUI:setVisible(Rank.UI.Node_model, false)
    GUI:setVisible(Rank.UI.ImageView, false)
    GUI:setVisible(Rank.UI.Button_looks, false)
    GUI:setVisible(Rank.UI.ImageView_Desc, true)
    local imgLevelX = GUI:getPositionX(Rank.UI.ImageView_level)
    local imgGuildX = GUI:getPositionX(Rank.UI.ImageView_guild)
    GUI:setPositionX(Rank.UI.ImageView_level, imgLevelX + 100)
    GUI:setPositionX(Rank.UI.ImageView_guild, imgGuildX + 90)
    GUI:Image_Create(Rank.UI.LayoutTItle, "ImageView_server", 210.00, -1.00, "res/custom/Rank/img_server.png")
end
...
function Rank.InitTypeBtn()
    ...
    if isInitRequest then
        SL:RequestRankData(Rank._showDataType) --请求排行榜数据 LUA_EVENT_RANK_DATA_UPDATE
    end

    --自定义全服战力排行
    if Rank._showDataType == 100 then
        Rank.UpdateGlobalRankList()
    end
end
...
--更新全服排行榜数据
function Rank.UpdateGlobalRankList()
    local level = SL:GetMetaValue("LEVEL")
    if level < 150 then
        sendmsg9("150级才可以查看！#249")
        return
    end
    local function httpCB(success, response)
        local data = { }
        if not success then
            sendmsg9("全区排行榜获取请求失败！")
            return
        end
        --SL:dump(response)
        local RankData = SL:JsonDecode(response, false)
        GUI:Text_setString(Rank.UI.Text_level, RankData.msg.myRank or "")
        GUI:Text_setString(Rank.UI.Text_guildName, RankData.msg.myGuild or "")
        if not RankData.msg.data or #RankData.msg.data == 0 then
            sendmsg9("未获取到排行榜数据！")
            return
        end
        data = RankData.msg.data
        GUI:removeAllChildren(Rank._rankList)
        for i, v in ipairs(data) do
        v.rank = i
        GUI:QuickCell_Create(Rank._rankList, "rank" .. i, 0, 0, 560, 40, function(parent)
            local cell = Rank.CreateGlobalListCell(parent, v)
            return cell
        end)
        end
    end
    ...
    --SL:HTTPRequestGet("http://159.75.153.98:880/api/Rank/GetRankData?UserID=" .. UserID.. "&ServerName=" .. ServerName, httpCB)
end
...
--加载全服行榜Cell
function Rank.CreateGlobalListCell(parent, data)
    GUI:LoadExport(parent, "rank/rank_global_cell")
    ...
    local showLvStr = data.Value or ""
    local showServerStr = data.Server or ""
    local guildText = ui.Text_3
    guildText = ui.Text_4
    GUI:Text_setString(ui.Text_3, showLvStr)
    GUI:Text_setString(ui.Text_2, showServerStr)
