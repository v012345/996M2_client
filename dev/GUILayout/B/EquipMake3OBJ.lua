...
EquipMake3OBJ.__cname = "EquipMake3OBJ"

EquipMake3OBJ.tab = {}
EquipMake3OBJ.where = 14
EquipMake3OBJ.itemGap = {0,76,48,20}
-------------------------------↓↓↓ UI操作 ↓↓↓---------------------------------------
function EquipMake3OBJ:main(objcfg,data,parent_ui)
    self.ui = parent_ui
    ...
    GUI:addOnClickEvent(self.ui.ButtonGo,function()
        ssrMessage:sendmsg(ssrNetMsgCfg.EquipMake3_Request)
    end)
    --注册消息
    self:registerMsg()
end

function EquipMake3OBJ:UpdateUI()
    --获取配置
    local zhangu_cfg = ssrRequireCsvCfg("cfg_zhangu")
    local equipInfo = SL:GetMetaValue("EQUIP_DATA", self.where)
    local equipIndex = equipInfo == nil and "null" or equipInfo["Index"]
    local heChengInfo = zhangu_cfg[equipIndex]
    ...
    local itemGap = EquipMake3OBJ.itemGap[xiaoHaoNum] or 5
    --自适应布局
...
------注册事消息
function EquipMake3OBJ:registerMsg()
...
function EquipMake3OBJ:ShowMoney(parentWidget, idName, ItemTable)
...
--刷新界面
function EquipMake3OBJ:Refresh()
