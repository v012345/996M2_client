...
function HeroSkill_Look_TradingBank.main()
    ...
    GUI:addOnClickEvent(HeroSkill_Look_TradingBank._ui.Button_setting, function()
...
function HeroSkill_Look_TradingBank.UpdateSkillCells()
    ...
    --已学的技能排除普攻
    local items = SL:CopyData(SL:GetMetaValue("T.H.LEARNED_SKILLS", true))
...
function HeroSkill_Look_TradingBank.UpdateSkillCell(skillID)
    ...
    local skill =  SL:GetMetaValue("T.H.SKILL_DATA", skillID) 
    if not skill then
        return
    end
    -- icon
    ...
    --熟练度 等级
    local strTrain = SL:GetMetaValue("T.H.SKILL_TRAIN_DATA", skillID) 
