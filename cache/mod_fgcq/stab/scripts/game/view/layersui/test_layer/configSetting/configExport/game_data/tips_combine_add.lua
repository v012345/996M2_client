...
function ui.init(parent)
	...
	-- Create Text
	local Text = GUI:Text_Create(Layout0, "Text", 43.00, 15.00, 16, "#ffffff", [[按附加属性显示（默认）]])
	...
	-- Create Text
	local Text = GUI:Text_Create(Layout1, "Text", 43.00, 15.00, 16, "#ffffff", [[按复古极品显示]])
	GUI:setAnchorPoint(Text, 0.00, 0.50)
	GUI:setTouchEnabled(Text, false)
	GUI:setTag(Text, -1)
	GUI:Text_enableOutline(Text, "#000000", 1)
	...
	-- Create Layout2
	local Layout2 = GUI:Layout_Create(panel_bg, "Layout2", 30.00, 210.00, 150.00, 30.00, false)
	GUI:setTouchEnabled(Layout2, true)
	GUI:setTag(Layout2, -1)

	-- Create img_sel
	local img_sel = GUI:Image_Create(Layout2, "img_sel", 12.00, 15.00, "res/public/btn_npcfh_05.png")
	GUI:setAnchorPoint(img_sel, 0.00, 0.50)
	GUI:setTouchEnabled(img_sel, false)
	GUI:setTag(img_sel, -1)

	-- Create img_unsel
	local img_unsel = GUI:Image_Create(Layout2, "img_unsel", 12.00, 15.00, "res/public/btn_npcfh_06.png")
	GUI:setAnchorPoint(img_unsel, 0.00, 0.50)
	GUI:setTouchEnabled(img_unsel, false)
	GUI:setTag(img_unsel, -1)

	-- Create Text
	local Text = GUI:Text_Create(Layout2, "Text", 43.00, 15.00, 16, "#ffffff", [[按基础属性白色字显示]])
	GUI:setAnchorPoint(Text, 0.00, 0.50)
	GUI:setTouchEnabled(Text, false)
	GUI:setTag(Text, -1)
	GUI:Text_enableOutline(Text, "#000000", 1)
