...
function TradingBankBuyTipsLayer:Init(data)
    local path = GUI:LoadInternalExport(self, "trading_bank/trading_bank_buy_tip")
    ...
    self._root.Panel_cancel:addTouchEventListener(function(sender, type)
        ...
        global.Facade:sendNotification(global.NoticeTable.Layer_TradingBankBuyTipsLayer_Close)
    end)
    self:RefUI(data)
    return true
end
