...
function PlayerBestRingLayer:InitGUI(data)
    SL:RequireFile(SLDefine.LUAFile.LUA_FILE_PLAYER_BESTRING_WIN32)
    ...
    for index, pos in ipairs(posSetting) do
        ...
        if data then
            ...
            local function mouseMoveCallBack()
                ...
                local onSellingOrRequiringMakeIndex = bagProxy:GetOnSellOrRepaire()
                if onSellingOrRequiringMakeIndex == data.MakeIndex then
                    return
                end
