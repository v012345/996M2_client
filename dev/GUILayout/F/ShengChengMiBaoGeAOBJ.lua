ShengChengMiBaoGeAOBJ = {}
ShengChengMiBaoGeAOBJ.__cname = "ShengChengMiBaoGeAOBJ"
ShengChengMiBaoGeAOBJ.ItemData = {
...
-------------------------------↓↓↓ UI操作 ↓↓↓---------------------------------------
function ShengChengMiBaoGeAOBJ:main(objcfg)
    ...
    ------------------------↓↓↓ 通讯后端 ↓↓↓------------------------
    ssrMessage:sendmsg(ssrNetMsgCfg.ShengChengMiBaoGeA_SyncResponse)
    ------------------------↑↑↑ 通讯后端 ↑↑↑------------------------
    ...
    GUI:addOnClickEvent(self.ui.Button_1, function()
        ssrMessage:sendmsg(ssrNetMsgCfg.ShengChengMiBaoGeA_Request,1)
    end)

    GUI:addOnClickEvent(self.ui.Button_2, function()
        ssrMessage:sendmsg(ssrNetMsgCfg.ShengChengMiBaoGeA_Request,2)
    end)
    -- 打开窗口缩放动画
    GUI:Timeline_Window1(self._parent)
end

function ShengChengMiBaoGeAOBJ:UpdateUI()
...
-------------------------------↓↓↓ 网络消息 ↓↓↓---------------------------------------
function ShengChengMiBaoGeAOBJ:SyncResponse(TongXingZheng, RiChongNum, arg3, data)
    self.TongXingZheng = TongXingZheng
    self.RiChongNum = RiChongNum

    SL:Print(TongXingZheng, RiChongNum)

    if GUI:GetWindow(nil, self.__cname) then
        self:UpdateUI()
    end
end
return ShengChengMiBaoGeAOBJ