...
function ui.init(parent)
	-- Create Panel_cell
	local Panel_cell = GUI:Layout_Create(parent, "Panel_cell", 605.00, 80.00, 605.00, 80.00, false)
	GUI:setAnchorPoint(Panel_cell, 1.00, 1.00)
	GUI:setTouchEnabled(Panel_cell, false)
	GUI:setTag(Panel_cell, 328)

	-- Create Image_bg
	local Image_bg = GUI:Image_Create(Panel_cell, "Image_bg", 8.00, 40.00, "res/public/1900000664.png")
	GUI:setAnchorPoint(Image_bg, 0.00, 0.50)
	GUI:setTouchEnabled(Image_bg, false)
	GUI:setTag(Image_bg, -1)

	-- Create Text_name
	local Text_name = GUI:Text_Create(Panel_cell, "Text_name", 82.00, 40.00, 16, "#ffffff", [[物品名称]])
	...
	-- Create Text_remain
	local Text_remain = GUI:Text_Create(Panel_cell, "Text_remain", 214.00, 40.00, 16, "#ffffff", [[剩余数量]])
	GUI:setAnchorPoint(Text_remain, 0.50, 0.50)
	GUI:setTouchEnabled(Text_remain, false)
	GUI:setTag(Text_remain, 101)
	GUI:Text_enableOutline(Text_remain, "#000000", 1)

	-- Create Text_min_num
	local Text_min_num = GUI:Text_Create(Panel_cell, "Text_min_num", 288.00, 40.00, 16, "#ffffff", [[最小数量]])
	...
	-- Create Text_coin
	local Text_coin = GUI:Text_Create(Panel_cell, "Text_coin", 350.00, 40.00, 16, "#ffffff", [[货币]])
	...
	-- Create Text_single
	local Text_single = GUI:Text_Create(Panel_cell, "Text_single", 405.00, 40.00, 16, "#ffffff", [[单价]])
	...
	-- Create Text_remain_total
	local Text_remain_total = GUI:Text_Create(Panel_cell, "Text_remain_total", 485.00, 40.00, 16, "#ffffff", [[剩余总价]])
	GUI:setAnchorPoint(Text_remain_total, 0.50, 0.50)
	GUI:setTouchEnabled(Text_remain_total, false)
	GUI:setTag(Text_remain_total, 103)
	GUI:Text_enableOutline(Text_remain_total, "#000000", 1)

	-- Create Text_oper
	local Text_oper = GUI:Text_Create(Panel_cell, "Text_oper", 562.00, 40.00, 16, "#ffffff", [[操作]])
	...
	-- Create Button_oper
	local Button_oper = GUI:Button_Create(Panel_cell, "Button_oper", 562.00, 40.00, "res/public/1900000652.png")
	GUI:setContentSize(Button_oper, 65, 27)
	GUI:setIgnoreContentAdaptWithSize(Button_oper, false)
	GUI:Button_setTitleText(Button_oper, "取消")
	GUI:Button_setTitleColor(Button_oper, "#ffffff")
	GUI:Button_setTitleFontSize(Button_oper, 14)
	...
	GUI:setContentSize(Image_line, 605, 2)
