local TakeOffEquipToHeroBagRequestCommand = class('TakeOffEquipToHeroBagRequestCommand', framework.SimpleCommand)

function TakeOffEquipToHeroBagRequestCommand:ctor()
end

function TakeOffEquipToHeroBagRequestCommand:execute(notification) ---脱装备
    ...
    LuaSendMsg(global.MsgType.MSG_CS_HUM_TAKEOFF_TO_HEROBAG, MakeIndex, pos, 0, 0, name, string.len(name))

end


return TakeOffEquipToHeroBagRequestCommand