...
EquipMake5OBJ.__cname = "EquipMake5OBJ"

EquipMake5OBJ.tab = {}
EquipMake5OBJ.where = 2
EquipMake5OBJ.itemGap = {0,76,48,20}
-------------------------------↓↓↓ UI操作 ↓↓↓---------------------------------------
function EquipMake5OBJ:main(objcfg,data,parent_ui)
    self.ui = parent_ui
    ...
    GUI:addOnClickEvent(self.ui.ButtonGo,function()
        ssrMessage:sendmsg(ssrNetMsgCfg.EquipMake5_Request)
    end)
    --注册消息
    self:registerMsg()
end

function EquipMake5OBJ:UpdateUI()
    --获取配置
    local baoshi_cfg = ssrRequireCsvCfg("cfg_baoshi")
    local equipInfo = SL:GetMetaValue("EQUIP_DATA", self.where)
    local equipIndex = equipInfo == nil and "null" or equipInfo["Index"]
    local heChengInfo = baoshi_cfg[equipIndex]
    ...
    local itemGap = EquipMake5OBJ.itemGap[xiaoHaoNum] or 5
    --自适应布局
...
------注册事消息
function EquipMake5OBJ:registerMsg()
...
function EquipMake5OBJ:ShowMoney(parentWidget, idName, ItemTable)
...
--刷新界面
function EquipMake5OBJ:Refresh()
