...
EquipMake1OBJ.__cname = "EquipMake1OBJ"

EquipMake1OBJ.tab = {}
EquipMake1OBJ.where = 12
EquipMake1OBJ.itemGap = {0,76,48,20}
-------------------------------↓↓↓ UI操作 ↓↓↓---------------------------------------
function EquipMake1OBJ:main(objcfg,data,parent_ui)
    self.ui = parent_ui
    ...
    GUI:addOnClickEvent(self.ui.ButtonGo,function()
        ssrMessage:sendmsg(ssrNetMsgCfg.EquipMake1_Request)
    end)
    --注册消息
    self:registerMsg()
end

function EquipMake1OBJ:UpdateUI()
    --获取配置
    local xueshi_cfg = ssrRequireCsvCfg("cfg_xueshi")
    local equipInfo = SL:GetMetaValue("EQUIP_DATA", self.where)
    local equipIndex = equipInfo == nil and "null" or equipInfo["Index"]
    local heChengInfo = xueshi_cfg[equipIndex]
    ...
    local itemGap = EquipMake1OBJ.itemGap[xiaoHaoNum] or 5
    --自适应布局
...
------注册事消息
function EquipMake1OBJ:registerMsg()
...
function EquipMake1OBJ:ShowMoney(parentWidget, idName, ItemTable)
...
--刷新界面
function EquipMake1OBJ:Refresh()
