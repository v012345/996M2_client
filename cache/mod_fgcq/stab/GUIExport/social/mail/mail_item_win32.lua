...
function ui.init(parent)
	-- Create item
	local item = GUI:Layout_Create(parent, "item", 300.00, 300.00, 187.00, 62.00, false)
	GUI:setChineseName(item, "邮件物品_组合")
	GUI:setTouchEnabled(item, true)
	GUI:setTag(item, 2)

	-- Create kuang01
	local kuang01 = GUI:Image_Create(item, "kuang01", 0.00, 0.00, "res/private/mail_win32/1900020062.png")
	GUI:setChineseName(kuang01, "邮件物品_选中底图1")
	GUI:setTouchEnabled(kuang01, false)
	GUI:setTag(kuang01, 52)

	-- Create kuang02
	local kuang02 = GUI:Image_Create(item, "kuang02", 0.00, 0.00, "res/private/mail_win32/1900020063.png")
	GUI:setChineseName(kuang02, "邮件物品_选中底图2")
	GUI:setTouchEnabled(kuang02, false)
	GUI:setTag(kuang02, 61)

	-- Create item_sender
	local item_sender = GUI:Text_Create(item, "item_sender", 15.00, 43.00, 12, "#a58e67", [[传奇团队]])
	...
	-- Create label_state
	local label_state = GUI:Text_Create(item, "label_state", 125.00, 43.00, 14, "#ff0500", [[已读]])
	...
	-- Create item_title
	local item_title = GUI:Text_Create(item, "item_title", 15.00, 18.00, 12, "#897867", [[系统奖励]])
	...
	-- Create btn_delete
	local btn_delete = GUI:Button_Create(item, "btn_delete", 165.00, 31.00, "res/private/mail_win32/1900020065.png")
