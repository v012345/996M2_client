...
function MiniMap.main()
    ...
    GUI:addOnClickEvent(MiniMap._ui["zdy_Button1"], function()
        MiniMap.UseIItem("随机传送石")
    end)
    GUI:addOnClickEvent(MiniMap._ui["zdy_Button2"], function()
        MiniMap.UseIItem("主城传送石")
    end)
    GUI:addOnClickEvent(MiniMap._ui["zdy_Button3"], function()
        ssrUIManager:OPEN(ssrObjCfg.TanCePanel)
    end)
    local mapName = SL:GetMetaValue("MAP_NAME")
    if mapName:find("斩将夺旗") then
        ssrMessage:sendmsg(ssrNetMsgCfg.ZhanJiangDuoQi_zjdqShowMap)
    end
...
------使用道具
function MiniMap.UseIItem(itemName)
    local itemId = SL:GetMetaValue("ITEM_INDEX_BY_NAME", itemName)
    if itemId then
        local itemResult = SL:UseItemByIndex(itemId)
        local msgData = {
            Type = 9,
            Msg = string.format("你的背包内没有[%s]！",itemName)
        }
        if not itemResult then
            SL:ShowLocalNoticeByType(msgData)
        end
    else
        SL:ShowLocalNoticeByType({
            Type = 9,
            Msg = string.format("系统没有物品[%s]!",itemName)
        })
    end
...
function MiniMap.AddCampaignFlags(data)
    if not data or not MiniMap._isOpen then
        return
    end
    local tFlagAreaData = data[1]
    local tGuildData = data[2] or {}
    if MiniMap._panelMap then
        local tFlagArea = {}
        local mapName = SL:GetMetaValue("MAP_NAME")
        if mapName:find("斩将夺旗") then
            --占领点范围
            tFlagArea = {
                ...
                --{"擂台", {190, 190, 10}, 50, 3000, 100, {190, 190}, type = "lt", timeLimit = 1200},
                --{"青龙旗帜3", {163, 253, 5}, 10, 1000, 100, {163, 247}, type = "ql3"},
                --{"青龙旗帜2", {127, 217, 5}, 10, 1000, 100, {127, 217}, type = "ql2"},
                --{"青龙旗帜1", {163, 217, 5}, 10, 1000, 100, {163, 217}, type = "ql1"},
                --{"白虎旗帜1", {163, 163, 5}, 10, 1000, 100, {163, 163}, type = "bh1"},
                --{"白虎旗帜2", {163, 127, 5}, 10, 1000, 100, {163, 127}, type = "bh2"},
                --{"白虎旗帜3", {127, 163, 5}, 10, 1000, 100, {127, 163}, type = "bh3"},
                --{"朱雀旗帜1", {217, 163, 5}, 10, 1000, 100, {217, 163}, type = "zq1"},
                --{"朱雀旗帜2", {253, 163, 5}, 10, 1000, 100, {253, 163}, type = "zq2"},
                --{"朱雀旗帜3", {217, 127, 5}, 10, 1000, 100, {217, 127}, type = "zq3"},
                --{"玄武旗帜3", {253, 217, 5}, 10, 1000, 100, {253, 217}, type = "xw3"},
                --{"玄武旗帜2", {217, 253, 5}, 10, 1000, 100, {217, 253}, type = "xw2"},
                --{"玄武旗帜1", {217, 217, 5}, 10, 1000, 100, {217, 217}, type = "xw1"}

                {"青龙旗帜3", {117, 89, 5}, 10, 1000, 100, {117, 89}, type = "ql3"},
                {"青龙旗帜2", {81, 125, 5}, 10, 1000, 100, {81, 125}, type = "ql2"},
                {"青龙旗帜1", {117, 125, 5}, 10, 1000, 100, {117, 125}, type = "ql1"},
                {"白虎旗帜1", {117, 179, 5}, 10, 1000, 100, {117, 179}, type = "bh1"},
                {"白虎旗帜2", {81, 179, 5}, 10, 1000, 100, {81, 179}, type = "bh2"},
                {"白虎旗帜3", {117, 215, 5}, 10, 1000, 100, {117, 215}, type = "bh3"},
                {"朱雀旗帜1", {171, 179, 5}, 10, 1000, 100, {171, 179}, type = "zq1"},
                {"朱雀旗帜2", {207, 179, 5}, 10, 1000, 100, {207, 179}, type = "zq2"},
                {"朱雀旗帜3", {171, 215, 5}, 10, 1000, 100, {171, 215}, type = "zq3"},
                {"玄武旗帜3", {171, 89, 5}, 10, 1000, 100, {171, 89}, type = "xw3"},
                {"玄武旗帜2", {207, 125, 5}, 10, 1000, 100, {207, 125}, type = "xw2"},
                {"玄武旗帜1", {171, 125, 5}, 10, 1000, 100, {171, 125}, type = "xw1"},
                {"擂台", {144, 144, 10}, 50, 3000, 100, {144, 144}, type = "lt", timeLimit = 1200},
            }
            local function _getRes(name)
                return "res/custom/ZhanJiangDuoQi/zjdq/" .. name .. ".png"
            end
            local _parent = MiniMap._ui["Node_zjdqShow"]
            GUI:removeAllChildren(_parent)
            local size = GUI:getContentSize(MiniMap._panelMiniMap)
            local w, h = size.width,size.height
            local bg = GUI:Image_Create(_parent, "Flags", 0, 0, "res/custom/ZhanJiangDuoQi/zjdqmap.png")
            GUI:setContentSize(bg,w,h)
            local container = GUI:Layout_Create(_parent, "FlagsContainer", 0, 0, w, h)
            local userId = SL:GetMetaValue("USER_ID")
            local guildName = SL:GetMetaValue("ACTOR_GUILD_NAME", userId)
            for j = 1, #tFlagArea do
                local areaName = tFlagArea[j][1] --旗帜点名称
                local tThisAreaData = tFlagAreaData[areaName] or {}
                local ownGuildName = tThisAreaData.GuildName or "" --当前占领
                local mapX = tFlagArea[j][2][1]
                local mapY = tFlagArea[j][2][2]
                local uiX = mapX / tFlagAreaData.MapW * w
                local uiY = (1 - mapY / tFlagAreaData.MapH) * h
                if ownGuildName ~= "" then
                    local colorIndex = tGuildData[ownGuildName].color
                    local area = GUI:Image_Create(container, "area" .. j, uiX, uiY, _getRes(tFlagArea[j].type .. "-" .. colorIndex))
                    --GUI:setAnchorPoint(area, 0.5, 0.5)
                    GUI:setAnchorPoint(area, 38 / 80, 1 - 60 / 90)
                else
                    local area = GUI:Image_Create(container, "area" .. j, uiX, uiY, _getRes(tFlagArea[j].type .. "-0"))
                    --GUI:setAnchorPoint(area, 0.5, 0.5)
                    GUI:setAnchorPoint(area, 38 / 80, 1 - 60 / 90)
                end
            end
...
ssrGameEvent:add(ssrEventCfg.OnZjdqShowMap, function(data)
    MiniMap.AddCampaignFlags(data)
end, MiniMap)
