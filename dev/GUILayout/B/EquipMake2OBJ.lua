...
EquipMake2OBJ.__cname = "EquipMake2OBJ"

EquipMake2OBJ.tab = {}
EquipMake2OBJ.where = 15
EquipMake2OBJ.itemGap = {0,76,48,20}
-------------------------------↓↓↓ UI操作 ↓↓↓---------------------------------------
function EquipMake2OBJ:main(objcfg,data,parent_ui)
    self.ui = parent_ui
    ...
    GUI:addOnClickEvent(self.ui.ButtonGo,function()
        ssrMessage:sendmsg(ssrNetMsgCfg.EquipMake2_Request)
    end)
    --注册消息
    self:registerMsg()
end

function EquipMake2OBJ:UpdateUI()
    --获取配置
    local shenyin_cfg = ssrRequireCsvCfg("cfg_shenyin")
    local equipInfo = SL:GetMetaValue("EQUIP_DATA", self.where)
    local equipIndex = equipInfo == nil and "null" or equipInfo["Index"]
    local heChengInfo = shenyin_cfg[equipIndex]
    ...
    local itemGap = EquipMake2OBJ.itemGap[xiaoHaoNum] or 5
    --自适应布局
...
------注册事消息
function EquipMake2OBJ:registerMsg()
...
function EquipMake2OBJ:ShowMoney(parentWidget, idName, ItemTable)
...
--刷新界面
function EquipMake2OBJ:Refresh()
