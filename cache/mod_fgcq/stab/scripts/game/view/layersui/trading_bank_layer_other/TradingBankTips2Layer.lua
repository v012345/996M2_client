...
function TradingBankTips2Layer:Init(data)
    local path = GUI:LoadInternalExport(self, "trading_bank_other/trading_bank_tips2")
    self._root = ui_delegate(self)
    self:InitAdapt()

    local ShowType = data.type or 1 -- 1 按钮  2滑块 
    local text = data.text or ""
    local btntext = data.btntext
    local titleImg = data.titleImg
    ...
    if text then
        ...
        if data.rich then
            self._root.Text_desc:setString("")
            local size = GUI:getContentSize(self._root.Image_1)
            local richText = GUI:RichText_Create(self._root.Image_1, "trade_richText", size.width / 2, size.height / 2, text, size.width-60, 20)
    ...
    if btntext then
        if type(btntext) == "table" then 
            if btntext[1] then 
                self._root.Button_1:setTitleText(btntext[1])
            end
            if btntext[2] then 
                self._root.Button_2:setTitleText(btntext[2])
                self._root.Button_2:setVisible(true)
                self._root.Button_2:setPositionX(378)
                self._root.Button_1:setPositionX(158)
            end
        else
    ...
    if titleImg then
        self._root.Image_title:loadTexture(titleImg)
    end
