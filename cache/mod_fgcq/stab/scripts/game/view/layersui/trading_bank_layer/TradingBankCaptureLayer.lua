...
function TradingBankCaptureLayer:Init(data)
    GUI:LoadInternalExport(self, "trading_bank/trading_bank_capture")
...
function TradingBankCaptureLayer:onButtonClick(sender, type)
    ...
    if name == "Button_2" then
        if self.m_selIndex == 1 then
            ...
            TradingBankProxy:doTrack(TradingBankProxy.UpLoadData.TraingSellRoleGenImageClick)
        elseif self.m_selIndex == 2 then
            local TradingBankProxy = global.Facade:retrieveProxy(global.ProxyTable.TradingBankProxy)
            TradingBankProxy:doTrack(TradingBankProxy.UpLoadData.TraingSellRoleButtonClick)
    ...
    elseif name == "Button_1" then
        if self.m_selIndex == 1 then
            global.Facade:sendNotification(global.NoticeTable.Layer_TradingBankCaptureLayer_Close)

            local TradingBankProxy = global.Facade:retrieveProxy(global.ProxyTable.TradingBankProxy)
            TradingBankProxy:doTrack(TradingBankProxy.UpLoadData.TraingSellRolePreClick)
        elseif self.m_selIndex == 2 then
            self:setPanel(1, true)
            self:setPanel(2, false)

            local TradingBankProxy = global.Facade:retrieveProxy(global.ProxyTable.TradingBankProxy)
            TradingBankProxy:doTrack(TradingBankProxy.UpLoadData.TraingSellRoleImageOkPreClick)
        end
    elseif name == "Button_4" then
        ...
        local TradingBankProxy = global.Facade:retrieveProxy(global.ProxyTable.TradingBankProxy)
        TradingBankProxy:doTrack(TradingBankProxy.UpLoadData.TraingSellRoleReGenImageClick)
    elseif name == "Button_3" then--提示
        self.TradingBankProxy:reqHelpText(self, {}, function(help)
...
function TradingBankCaptureLayer:onImageClic2(sender, type)
    ...
    global.Facade:sendNotification(global.NoticeTable.Layer_TradingBankLookTexture_Open, sender)
end
-------------------
...
function TradingBankCaptureLayer:uploadImg()
    ...
    for position, vec in pairs(self.m_files) do
        for i, path in ipairs(vec) do
            local val = {
                position = position,
                file = self.m_fileDatas[path]
            }
            table.insert(self.m_uploadImageList, val)
        end
    end
    self.TradingBankProxy:uploadImg(self, self.m_uploadImageList[self.m_uploadImgIndex] or {}, handler(self, self.ResUploadImg))
end
...
function TradingBankCaptureLayer:onSellRole()
    ...
    self.TradingBankProxy:sellRole(self, t, handler(self, self.ResSellRole))
end
function TradingBankCaptureLayer:ResSellRole(success, response, code)
    if success then
        local data = cjson.decode(response)
        if data.code == 200 then--寄售成功
        ...
        elseif data.code == 30105 then --xx分钟之内有取回操作
            global.Facade:sendNotification(global.NoticeTable.Layer_TradingBankTipsLayer_Open, { callback = function()
        ...
        self.m_progress = 0
        self:setPanel(3, false)
        global.Facade:sendNotification(global.NoticeTable.SystemTips, GET_STRING(600000119))
    end
end
...
function TradingBankCaptureLayer:ResUploadImg(success, response, code)
...
function TradingBankCaptureLayer:CaptureImg()
    ...
    local message = {
        { open = NT.Layer_Player_Open, close = NT.Layer_Player_Close, extent = { extent = SLDefine.PlayerPage.MAIN_PLAYER_LAYER_EQUIP }, mediator = "PlayerFrameMediator", position = "equip_images" },
        { open = NT.Layer_Player_Open, close = NT.Layer_Player_Close, extent = { extent = SLDefine.PlayerPage.MAIN_PLAYER_LAYER_BASE_ATTRI }, mediator = "PlayerFrameMediator", position = "status_images" },
        { open = NT.Layer_Player_Open, close = NT.Layer_Player_Close, extent = { extent = SLDefine.PlayerPage.MAIN_PLAYER_LAYER_EXTRA_ATTRO }, mediator = "PlayerFrameMediator", position = "attribute_images" },
        { open = NT.Layer_Player_Open, close = NT.Layer_Player_Close, extent = { extent = SLDefine.PlayerPage.MAIN_PLAYER_LAYER_SKILL }, mediator = "PlayerFrameMediator", position = "skill_images" },
        { open = NT.Layer_Player_Open, close = NT.Layer_Player_Close, extent = { extent = SLDefine.PlayerPage.MAIN_PLAYER_LAYER_TITLE }, mediator = "PlayerFrameMediator", position = "title_images" },
        { open = NT.Layer_Player_Open, close = NT.Layer_Player_Close, extent = { extent = SLDefine.PlayerPage.MAIN_PLAYER_LAYER_SUPER_EQUIP }, mediator = "PlayerFrameMediator", position = "clothes_images" },
        -- {open = NT.Layer_Bag_Open, close = NT.Layer_Bag_Close,  extent = {pos = {x = 0, y = 0}},mediator = "BagLayerMediator",node = "panel",position = "bag_images"},
        --{ open = NT.Layer_NPC_Storage_Open, close = NT.Layer_NPC_Storage_Close, extent = { noShowBag = true }, mediator = "NPCStorageMediator", position = "warehouse_images" },
    ...
    for i = 1, pagenum do
        ...
        { open = NT.Layer_Bag_Open, close = NT.Layer_Bag_Close, extent = {pos = { x = 0, y = 0 }, bag_page = i }, mediator = "BagLayerMediator", node = "panel", position = "bag_images" })
    end
    -------------------------------------------------------仓库
    local NPCStorageProxy = global.Facade:retrieveProxy(global.ProxyTable.NPCStorageProxy)
    local storemax = NPCStorageProxy:GetMaxPage()
    for i = 1,storemax do
        local ebs = { open = NT.Layer_NPC_Storage_Open, close = NT.Layer_NPC_Storage_Close, extent = { noShowBag = true, initPage = i }, mediator = "NPCStorageMediator", position = "warehouse_images" }
    ...
    if activeState then
        ...
        }, mediator = "PlayerBestRingLayerMediator", position = "equip_images" }
        table.insert(message, ebs)
    end
    ...
    if HeroPropertyProxy:IsHeroOpen() then
        ...
        if PlayerPropertyProxy:getIsMakeHero() then
            ...
            local heromessage = {
                { open = NT.Layer_Player_Open_Hero, close = NT.Layer_Player_Close_Hero, extent = { extent = SLDefine.PlayerPage.MAIN_PLAYER_LAYER_EQUIP }, mediator = "HeroFrameMediator", position = "hero_equip_images" },
                { open = NT.Layer_Player_Open_Hero, close = NT.Layer_Player_Close_Hero, extent = { extent = SLDefine.PlayerPage.MAIN_PLAYER_LAYER_BASE_ATTRI }, mediator = "HeroFrameMediator", position = "hero_status_images" },
                { open = NT.Layer_Player_Open_Hero, close = NT.Layer_Player_Close_Hero, extent = { extent = SLDefine.PlayerPage.MAIN_PLAYER_LAYER_EXTRA_ATTRO }, mediator = "HeroFrameMediator", position = "hero_attribute_images" },
                { open = NT.Layer_Player_Open_Hero, close = NT.Layer_Player_Close_Hero, extent = { extent = SLDefine.PlayerPage.MAIN_PLAYER_LAYER_SKILL }, mediator = "HeroFrameMediator", position = "hero_skill_images" },
                { open = NT.Layer_Player_Open_Hero, close = NT.Layer_Player_Close_Hero, extent = { extent = SLDefine.PlayerPage.MAIN_PLAYER_LAYER_TITLE }, mediator = "HeroFrameMediator", position = "hero_title_images" },
                { open = NT.Layer_Player_Open_Hero, close = NT.Layer_Player_Close_Hero, extent = { extent = SLDefine.PlayerPage.MAIN_PLAYER_LAYER_SUPER_EQUIP }, mediator = "HeroFrameMediator", position = "hero_clothes_images" },
                { open = NT.Layer_HeroBag_Open, close = NT.Layer_HeroBag_Close, extent = {pos = { x = 0, y = 0 }}, mediator = "HeroBagLayerMediator", node = "panel", position = "hero_bag_images" },
            }
    ...
    if activeState then
        ...
        }, mediator = "HeroBestRingLayerMediator", position = "hero_equip_images" }
        table.insert(message, ebs)
    end
                ...
                if node and self:CaptureNode(position..repIndex..".png",node) then
                    ...
                    performWithDelay(self._root.Panel_p,function ()
                        ...
                        if FileUtils:isFileExist(path) then
                            ...
                            self.m_fileDatas[path] = fileData
                        else
                            CloseFunc()
                            return 
                        end
