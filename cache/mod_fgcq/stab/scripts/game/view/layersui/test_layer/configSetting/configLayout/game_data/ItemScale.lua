...
function ItemScale.main(parent, param)
    if not parent then
        return
    end

    loadConfigSettingExport(parent, "game_data/item_scale")

    ItemScale._ui = GUI:ui_delegate(parent)
    ...
    local scaleMobile, scalePC = tonumber(slices[1]) or 1, tonumber(slices[2]) or 1
    local input_mobile = ItemScale._ui["input_mobile"]
    local input_pc = ItemScale._ui["input_pc"]
    ...
    GUI:addOnClickEvent(ItemScale._ui["Button_sure"], function()
        GUI:delayTouchEnabled(ItemScale._ui["Button_sure"], 0.5)
