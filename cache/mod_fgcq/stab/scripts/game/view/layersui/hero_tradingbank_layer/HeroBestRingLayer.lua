...
function HeroBestRingLayer:InitGUI(data)
    SL:RequireFile(SLDefine.LUAFile.LUA_FILE_TRADE_HERO_BESTRING)
    HeroBestRing_Look_TradingBank.main(data)
    self._root = HeroBestRing_Look_TradingBank._ui.Panel_1

    local itemPanelSz = GUI:getContentSize(HeroBestRing_Look_TradingBank._ui.Panel_touch)
    HeroBestRing_Look_TradingBank._Panel_width = itemPanelSz.width
    HeroBestRing_Look_TradingBank._Panel_height = itemPanelSz.height

    GUI:setSwallowTouches(HeroBestRing_Look_TradingBank._ui.Panel_touch, false)

    self:InitUI()
    self:RefreshEquipList()
    self._CaptureNode = HeroBestRing_Look_TradingBank._ui.Panel_1
end

function HeroBestRingLayer:InitHideIconPos()
    self._hideIconPos = {}
    for i, v in ipairs(HeroBestRing_Look_TradingBank.posSetting or {}) do
...
function HeroBestRingLayer:InitUI()
    ...
    global.mouseEventController:registerMouseButtonEvent(HeroBestRing_Look_TradingBank._ui.Panel_touch,
    {
...
function HeroBestRingLayer:RefreshEquipList()
    local panelItems = HeroBestRing_Look_TradingBank._ui.Panel_items
    local posSetting = HeroBestRing_Look_TradingBank.posSetting
    ...
    for index, pos in ipairs(posSetting) do
        ...
        if not self._hideIconPos[pos]  then
            if  HeroBestRing_Look_TradingBank._HideDefaultIcon and HeroBestRing_Look_TradingBank._HideDefaultIcon[index] then 
        ...
        if data then
            ...
            goodItem = HeroBestRing_Look_TradingBank.CreateEquipItemCallBack(goodItem, info)
...
function HeroBestRingLayer:GetItemBagEmptyPos(touchPos)
    local x = touchPos.x
    local y = touchPos.y
    local panelWorldPos = HeroBestRing_Look_TradingBank._ui.Panel_touch:getWorldPosition()
    local posXInPanel = x - panelWorldPos.x
    local posYInPanel = panelWorldPos.y - y
    if posXInPanel >= HeroBestRing_Look_TradingBank._Panel_width or posXInPanel <= 0 then
        return nil
    end

    if posYInPanel >= HeroBestRing_Look_TradingBank._Panel_height or posYInPanel <= 0 then
        return nil
    end
    ...
    for i, pos in ipairs(HeroBestRing_Look_TradingBank.posSetting or {}) do
        local equipPanel = panelItems:getChildByName("Node_" .. pos)
        if equipPanel then
            nodeRect.x = equipPanel:getPositionX() - global.MMO.BEST_RING_ITEM_WIDTH / 2
            nodeRect.y = HeroBestRing_Look_TradingBank._Panel_height - equipPanel:getPositionY() - global.MMO.BEST_RING_ITEM_HEIGHT / 2
...
function HeroBestRingLayer:RefreshAddTouchSize()
    if HeroBestRing_Look_TradingBank._ui.Panel_touch and not tolua.isnull(HeroBestRing_Look_TradingBank._ui.Panel_touch) then
        local itemPanelSz = HeroBestRing_Look_TradingBank._ui.Panel_touch:getContentSize()
        HeroBestRing_Look_TradingBank._Panel_width = itemPanelSz.width
        HeroBestRing_Look_TradingBank._Panel_height = itemPanelSz.height
    end
end
