ItemTips = {}
ItemTips.itemConfig = ssrRequireCsvCfg("cfg_itemTipsImages")
...
local _TextSize = 18
local _DefaultFSize = SL:GetMetaValue("WINPLAYMODE") and 14 or 16     -- 官方默认字号
...
----自写，非官方
----自定义添加装备tips上的控件
--function ItemTips.CustomPushItem(parent, childItem)
...
function ItemTips.PushItem(parent, childItem)
    ...
    elseif posy < 0 and posy - anchorPoint.y * childrenSz.height >= -rootPanelSz.height then
        --位置在底部之上
...
function ItemTips.GetModeStr(itemData)
    ...
    -- elseif itemData.StdMode == 25 then
    --     --护身符及毒药
...
function ItemTips.GetAttStr(itemData, diff)
    ...
    if basicAttrShow and next(basicAttrShow) then
        ...
        local titleStr = "<img src='res/tips/4.png'/>"
        table.insert(
                str,
                {
                    str = titleStr
    ...
    for _, v in pairs(basicAttrShow) do
        ...
        if exAttShow and exAttShow[v.id] then
             ...
             if itemData.Name == "潜龙阴阳石" then
                 local  AttrStr = exAttShow[v.id].value
                 if string.sub(AttrStr, 1, 2) == "+-" then
                     AttrStr = string.sub(AttrStr, 2)
                     oneStr = oneStr .. string.format("　[基因改造]:%s↓", AttrStr)
                     color = 58
                 else
                     oneStr = oneStr .. string.format("　[基因改造]:%s↑", AttrStr)
                     color = 1039
    ...
    if yuansuAttrShow and next(yuansuAttrShow) then
        ...
        local yuansuTitle = "<img src='res/tips/20.png'/>"
        table.insert(
                str2,
                {
                    str = yuansuTitle
...
function ItemTips.ParseExAddAttr(attr, needAttrList)
    ...
    if attrShow and next(attrShow) then
        ...
        --local titleStr = string.format("<font color='%s'>%s</font>", SL:GetHexColorByStyleId(titleColor), titleName)
        local titleStr = "<img src='res/tips/5.png'/>"
    ...
    for p, d in pairs(data.abil or {}) do
        ...
        for k, v in pairs(isShowAttr and attList or {}) do
            ...
            for i, a in pairs(v) do
                ...
                local value = a.value
                if (attConfig and attConfig.type == 2) or a.type == 2 then
                    --万分比除100
...
function ItemTips.GetLimitStr(itemData, from)
    ...
    if itemData.StdMode == 4 then
        ...
        if (needJob >= 0 and needJob <= 2) then
            --人物
            conditionStr = "人物职业：%s"
            jobName = jobNameList[1 + needJob]
        elseif needJob >= 3 and needJob <= 5 then
            --英雄
        ...
        elseif needJob >= 9 and needJob <= 20 then
            -- 多职业
            conditionStr = "人物职业：%s"
            needJob = needJob - 4
    ...
    if suitConfigs and next(suitConfigs) then
        for cfgIdx, suitConfig in ipairs(suitConfigs) do
            if suitConfig and next(suitConfig) then
                ...
                if suitConfig.desc and suitConfig.desc ~= "" then
                    -- 套装无描述不显示
                    local meetCount = 0
                    ...
                    if titleStr and string.len(titleStr) > 0 then
                        ...
                        local showTitleFormat = string.format(
...
function ItemTips.GetSuitPanel(suitConfig, itemData, idx)
    ...
    if descs and descs.desc and next(descs.desc) then
        --这里不支持top  bottom
...
function ItemTips.RefreshItemPosition(tips, listView, size)
    ...
    if not isTipsOutSideBtn then
        ...
        for i = 1, 3 do
            ...
            if btn then
                ...
                local posY = listHeight - (index - 1) * 42 - btnSz.height / 2 + movePosY
                local itemSzW = 0
                if index > 1 then
                ...
                --tips按钮的位置
                GUI:setPosition(btn, posX + movePosX + rightSpace - 50, posY - 118)
                index = index + 1
            end
        end
    end
    ...
    --重置tips的大小
    --GUI:setContentSize(listView, listWidth + tipExtraWidth - 20, listHeight - 15)
    GUI:setPositionY(listView, math.floor(GUI:getPositionY(listView) + 15))
    GUI:setContentSize(listView, listWidth + tipExtraWidth - 20, listHeight)
    GUI:setContentSize(tips, listWidth + tipExtraWidth - 20, listHeight)
...
function ItemTips.ResetEquipPosition()
    ...
    --处理为偶数
    local function ensureEven(number)
        if number % 2 == 0 then
            return number  -- 如果数字是偶数，直接返回
        else
            return number + 1  -- 如果数字是奇数，加1后返回
        end
    end

    GUI:setContentSize(ItemTips._PList, ensureEven(width), ensureEven(maxHeight))
    width = 0
...
--根据装备名字获取图片
function ItemTips.getEquipmentImage(equipmentName)
    local images = ItemTips.itemConfig[equipmentName]
    local defaultImages = {
    "putong",
    "QiTa",
    "TeShu"
    }
    local path1 = "res/tips/topBg/"
    local path2 = "res/tips/topType/leixing/"
    local path3 = "res/tips/topType/buwei/"
    local result1, result2, result3 = "","",""
    if images then
        if images.bg then
            result1 = path1 .. images.bg
        else
            result1 = path1..defaultImages[1]
        end
        if images.leftIcon then
            result2 = path2 .. images.leftIcon
        else
            result2 = path2..defaultImages[2]
        end
        if images.rightIcon then
            result3 = path3 .. images.rightIcon
        else
            result3 = path3..defaultImages[3]
        end
        return result1 .. ".png", result2 .. ".png", result3 .. ".png"
    else
        return path1..defaultImages[1] .. ".png", path2..defaultImages[2] .. ".png", path3..defaultImages[3] .. ".png"
    end
end
------------  装备tips  ------------------------------------------------
function ItemTips.CreateEquipPanel(data, itemData, isWear, panelInsertIndex)
    ...
    local contentPanel = GUI:Layout_Create(scrollView, "contentPanel", 15, 0, maxWidth, 1)
    ...
    --end
    local path1, path2, path3 = ItemTips.getEquipmentImage(name)
    --装备顶部背景图
    local itemTipsTopBackGround = GUI:Image_Create(contentPanel, "itemTipsTopBackGround", 0, 0, path1)
    GUI:setPosition(itemTipsTopBackGround, -12, -102)
    GUI:setLocalZOrder(itemTipsTopBackGround, 1)
    --ItemTips.PushItem(contentPanel, itemTipsTopBackGround)
    local itemTipsTypePanel = GUI:Layout_Create(contentPanel, "itemTipsTypePanel", 0, 0, 150, 28)
    GUI:setAnchorPoint(itemTipsTypePanel, 0.5, 0.5)
    GUI:setLocalZOrder(itemTipsTypePanel, 2)
    --类别1
    local itemTipsTypeLeft = GUI:Image_Create(itemTipsTypePanel, "itemTipsTypeLeft", 0, 0, path2)
    GUI:setContentSize(itemTipsTypeLeft, 71, 28)
    --类别2
    local itemTipsTypeRight = GUI:Image_Create(itemTipsTypePanel, "itemTipsTypeRight", 76, 0, path3)
    GUI:setContentSize(itemTipsTypeRight, 71, 28)
    -- 装备名称
    ...
    local r_name = GUI:Text_Create(contentPanel, "r_name", 0, 0, _TextSize, SL:GetHexColorByStyleId(color), name)
    --GUI:Text_setFontName(r_name, "fonts/10001.ttf")
    ItemTips.PushItem(contentPanel, r_name)

    local itemDescs = GUIFunction:GetParseItemDesc(itemData.Desc)
    --PC的显示偏移
    if SL:GetMetaValue("WINPLAYMODE") then
        local rootPanelSz = GUI:getContentSize(contentPanel)
        GUI:setContentSize(contentPanel, rootPanelSz.width, rootPanelSz.height + 4)
    end
    -- 顶部描述
    local topDescs = itemDescs.top_desc
    if topDescs then
        local topDescsOffsetH = 0
        if SL:GetMetaValue("WINPLAYMODE") then
            topDescsOffsetH = 1
        else
            topDescsOffsetH = 10
        ...
        local rootPanelSz = GUI:getContentSize(contentPanel)
        GUI:setContentSize(contentPanel, rootPanelSz.width, rootPanelSz.height - topDescsOffsetH)
        removeLastLine()
        ItemTips.PushItem(contentPanel, ItemTips.CreateIntervalPanel(contentPanel, _DefaultSpace / 2))
        pushDescItem(topDescs, 1)
        rootPanelSz = GUI:getContentSize(contentPanel)
        GUI:setContentSize(contentPanel, rootPanelSz.width, rootPanelSz.height - 24)
        --ItemTips.PushItem(contentPanel, ItemTips.CreateIntervalPanel(contentPanel, _DefaultSpace, true))
    end
    ...
    local icon_bg = GUI:Image_Create(contentPanel, "icon_bg", 0, -80, res)
    ...
    if SL:GetMetaValue("SERVER_OPTIONS", SW_KEY_ITEMTIPS_TOUBAO_SHOW) == 1 then
        ...
        if touBaoStr then
            local rich_tb = GUI:RichText_Create(contentPanel, "rich_tb", 0, -6, touBaoStr, richWidth, _setFontSize or _DefaultFSize, "#FF0500", vspace, nil, fontPath)
    ...
    if upAttrRichs and #upAttrRichs > 0 then
        for i, rich_att in ipairs(upAttrRichs) do
            local image = GUI:Image_Create(rich_att, "up_attr_tag_" .. i, upAttrMaxWidth - 60, 1, "res/public/btn_szjm_01_3.png")
    ...
    local currSize = GUI:getContentSize(ListBg)
    GUI:setContentSize(itemTipsTopBackGround, currSize.width - 4, 100)
    --装备名字
    local equipNameOffsetY = 0
    local rich_att_1_OffsetY = 0
    --电脑
    if SL:GetMetaValue("WINPLAYMODE") then
        equipNameOffsetY = 5
        rich_att_1_OffsetY = -3
    else
        equipNameOffsetY = 3
        rich_att_1_OffsetY = -8
    end
    GUI:setPosition(r_name, currSize.width / 2 - 16, GUI:getPositionY(r_name) - equipNameOffsetY)
    GUI:setAnchorPoint(r_name, 0.5, 0.5)

    --处理顶部描述
    if topDescs then
        local desc1 = GUI:getChildByName(contentPanel, "rich_att_1")
        if desc1 then
            GUI:setAnchorPoint(desc1, 0.5, 0.5)
            GUI:setPosition(desc1, currSize.width / 2 - 16, GUI:getPositionY(desc1) + rich_att_1_OffsetY)
        end
    end

    --处理类别位置
    GUI:setPosition(itemTipsTypePanel, currSize.width / 2 - 13, GUI:getPositionY(itemTipsTypePanel) - 84)
    ...
    local ListBgSize = GUI:getContentSize(ListBg)
    GUI:setContentSize(ListBg, ListBgSize.width, ListBgSize.height + 15)
    --特效自适应
    ItemTips.AddFrameEffect(ListBg, itemDescs)
end
function ItemTips.getItemImage(ItemName)
    local images = ItemTips.itemConfig[ItemName]
    local defaultImages = {
    "putong",
    "QiTa",
    "Cailiao"
...
function ItemTips.CreateItemPanel(data, itemData)
    ...
    local contentPanel = GUI:Layout_Create(scrollView, "contentPanel", 10, 0, 47, 1)
    GUI:setAnchorPoint(contentPanel, 0, 0)
    ItemTips.contentPanel = contentPanel

    local color = (itemData.Color and itemData.Color > 0) and itemData.Color or 255
    local name = itemData.Name or ""
    local path1, path2, path3 = ItemTips.getItemImage(name)
    local itemTipsTopBackGround = GUI:Image_Create(contentPanel, "itemTipsTopBackGround", 0, 0, path1)
    GUI:setPosition(itemTipsTopBackGround, -8, -118)
    GUI:setLocalZOrder(itemTipsTopBackGround, 1)
    --ItemTips.PushItem(contentPanel, itemTipsTopBackGround)
    ...
    if not SL:GetMetaValue("WINPLAYMODE") and data.from == ItemFrom.BAG then
        if IsOpenSwitch(3, itemData.StdMode) then
            if not (itemData.StdMode == 40 and itemData.Shape == 15) then
                -- 宝箱不显示使用
    ...
    --道具背景的宽度大小
    if SL:GetMetaValue("WINPLAYMODE") then
        GUI:setContentSize(ListBg, maxWidth + 60, listH) --Pc端的道具tips宽度调整
    else
        GUI:setContentSize(ListBg, maxWidth + 100, listH)
    end
    ...
    local currSize = GUI:getContentSize(ListBg)
    GUI:setContentSize(itemTipsTopBackGround, currSize.width - 2, 100)
    --装备名字
    local equipNameOffsetY = 0
    local rich_att_1_OffsetY = 0
    --电脑
    if SL:GetMetaValue("WINPLAYMODE") then
        equipNameOffsetY = 21
        rich_att_1_OffsetY = -17
    else
        equipNameOffsetY = 18
        rich_att_1_OffsetY = -22
    end
    GUI:setPosition(r_name, currSize.width / 2 - 16, GUI:getPositionY(r_name) - equipNameOffsetY)
    GUI:setAnchorPoint(r_name, 0.5, 0.5)
    ...
    --处理类别位置
    GUI:setPosition(itemTipsTypePanel, currSize.width / 2 - 8, GUI:getPositionY(itemTipsTypePanel) - 100)
