...
function HeroEquip_Look.main(data)
    ...
    GUI:LoadExport(parent, "hero_look/hero_equip_node_win32")
    ...
    GUI:addOnClickEvent(HeroEquip_Look._ui.Best_ringBox,function()
        --首饰盒是否开启
...
function HeroEquip_Look.InitEquipEvent()
    ...
    for _, pos in ipairs(posSetting) do
        ...
        HeroEquip_Look.InitPanelMouseEvent(equipPanel, pos)
    end
end

function HeroEquip_Look.InitPanelMouseEvent(equipPanel, pos)
...
function HeroEquip_Look.RefreshPlayerBestRingsOpenState(data)
    ...
    local function mouseMoveCallBack(touchPos)
        if not GUI:getVisible(HeroEquip_Look._ui.Best_ringBox) then 
            return
        end
        local bestRingsName = SL:GetMetaValue("SERVER_OPTION", "SndaItemBoxName") or "首饰盒"
        if SL:CheckNodeCanCallBack(HeroEquip_Look._ui.Best_ringBox, touchPos) and bestRingsName ~= "" then
            local activeState = SL:GetMetaValue("BEST_RING_OPENSTATE", HeroEquip_Look.RoleType.OtherHero)
            ...
            local worldPos = GUI:getWorldPosition(HeroEquip_Look._ui.Best_ringBox)
            worldPos.x = GUI:getContentSize(HeroEquip_Look._ui.Best_ringBox).width / 2 + worldPos.x
            GUI:ShowWorldTips(tipsStr, worldPos, GUI:p(0.5, 1))
        end
    end

    local function leaveItem()
        local nodeTips = GUI:getChildByName(HeroEquip_Look._ui.Best_ringBox,"TIPS")
        if nodeTips then
    ...
    GUI:addMouseMoveEvent(HeroEquip_Look._ui.Best_ringBox,
        {
...
function HeroEquip_Look.RefreshBestRingBox()
    SL:scheduleOnce(HeroEquip_Look._ui.Best_ringBox,function()
        ...
        GUI:Image_loadTexture(HeroEquip_Look._ui.Image_box, SLDefine.PATH_RES_PRIVATE .. "player_best_rings_ui/player_best_rings_ui_win32/" .. texture)
        -- 重置尺寸
        GUI:setIgnoreContentAdaptWithSize(HeroEquip_Look._ui.Image_box, true)
        HeroEquip_Look.RefreshPlayerBestRingsOpenState()
    end,0.1)
end
