...
function GameEnvManager:LoadGameInfo()
    ...
    local function callback()
        ...
        if count >= limit and (nil == self:GetChannelID() or 0 == string.len(self:GetChannelID())) then
            ...
            -- 联系客服
            self:ReportError("网络异常，请重试！[-1]", function()
                self:LoadGameInfo()
            end)
        end
    end
...
function GameEnvManager:parseModList(jsonData)
    ...
    -- 审核服检测
    local currVersion   = jsonData.currVersion
    local reviewVersion = jsonData.reviewVersion
    local versionCode   = tonumber(self:GetAPKVersionCode())
    if versionCode and currVersion and string.len(currVersion) > 0 then
        currVersion     = tonumber(currVersion) or 0
        reviewVersion   = tonumber(reviewVersion)

        if versionCode < currVersion then
            -- 已废弃，不读取
        else
            -- 提审/测试id不为空 比对id是否相同
            if reviewVersion and versionCode == reviewVersion then
    ...
    if resURL and string.len(resURL) > 0 then
...
function GameEnvManager:CheckUpdate()
    ...
    local rootDomain        = self:GetRootDomain()
...
function GameEnvManager:launch()
    ...
    local function loadGame()
        ...
        if cc.PLATFORM_OS_WINDOWS == cc.Application:getInstance():getTargetPlatform() then
            ...
            glview:setDesignResolutionSize(global.DesignSize_Win.width, global.DesignSize_Win.height, designPolicy)
