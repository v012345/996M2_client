...
local LookHeroEquipLayerMediator = class("LookHeroEquipLayerMediator", BaseUIMediator)
LookHeroEquipLayerMediator.NAME  = "LookHeroEquipLayerMediator"

function LookHeroEquipLayerMediator:ctor()
    LookHeroEquipLayerMediator.super.ctor( self )
    self._layer = nil
end

function LookHeroEquipLayerMediator:listNotificationInterests()
    local noticeTable = global.NoticeTable
    return {
            noticeTable.Layer_Look_Player_Equip_Open_Hero,
            noticeTable.Layer_Look_Player_Child_Del_Hero,
    }
end

function LookHeroEquipLayerMediator:handleNotification(notification)
    local notices  = global.NoticeTable
    local noticeName = notification:getName()
    local noticeData = notification:getBody()
    
    if noticeName == notices.Layer_Look_Player_Equip_Open_Hero then
...
function LookHeroEquipLayerMediator:OpenLayer(noticeData)
    if not self._layer then
        local path = "hero_look_layer/HeroEquipLayer"
        if global.isWinPlayMode then
            path = "hero_look_layer/HeroEquipLayer_win32"
        end
...
function LookHeroEquipLayerMediator:CloseLayer()
    -- 自定义组件挂接
    local componentData = {
        index = global.SUIComponentTable.PlayerEquipO_hero
    }
...
return LookHeroEquipLayerMediator
