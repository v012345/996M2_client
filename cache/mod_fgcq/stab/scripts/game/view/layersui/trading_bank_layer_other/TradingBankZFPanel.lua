        ...
        if self._buyType == BuyType.ORDER then
            local function checkAuthentication()
                self.OtherTradingBankProxy:checkAuthentication(self,function(code, data, msg)
                    ...
                    if code == 200 then
                        if data then
                            self.OtherTradingBankProxy:reqcommodityInfo(self, { commodity_id = self.m_goodsData.id }, function(code2, data2, msg2)
                            if code2 == 200 then
                                dump(data2,"商品信息")
                                local mainServerId = self.LoginProxy:GetMainSelectedServerId()
                                local goodMainServerId =  tonumber(data2.mainServerId)
                                ...
                                if mainServerId and goodMainServerId then 
                                    ...
                                    if tonumber(self.LoginProxy:GetSelectedServerId()) == tonumber(data2.serverId) then 
                                        sameServer = true 
                                    end
                                end
