...
function TradingBankCaptureMaskLayer:Init(data)
    GUI:LoadInternalExport(self, "trading_bank/trading_bank_capture_mask")
    ...
    GUI:setVisible(self._ui.Image_tips, false)

    if data and data.TipsStr then
        GUI:setVisible(self._ui.Image_tips, true)
        GUI:setPosition(self._ui.Image_tips, winSizeW / 2, winSizeH / 2)
        GUI:Text_setString(self._ui.Text_tips, data.TipsStr)
    end
    return true
end
