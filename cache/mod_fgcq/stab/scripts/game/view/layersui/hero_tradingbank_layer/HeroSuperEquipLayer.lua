...
function HeroSuperEquipLayer:InitGUI(data)
    SL:RequireFile(SLDefine.LUAFile.LUA_FILE_TRADE_HERO_SUPER_EQUIP)
    HeroSuperEquip_Look_TradingBank.main(data)
    self._root = HeroSuperEquip_Look_TradingBank._ui.Panel_1
...
function HeroSuperEquipLayer:InitEquipLayerEvent()
    local posSetting = HeroSuperEquip_Look_TradingBank.posSetting
    local function GetEquipDataByPos(equipPos, equipList)
        ...
        if equipList then
            equipItems = TradingBankLookPlayerProxy:GetLookPlayerItemDataList_Hero(equipPos)
        else
            equipItems = TradingBankLookPlayerProxy:GetLookPlayerItemDataByPos_Hero(equipPos, true)
        end
        return equipItems
    end
...
function HeroSuperEquipLayer:InitEquipLayer()
    local TradingBankLookPlayerProxy = global.Facade:retrieveProxy(global.ProxyTable.TradingBankLookPlayerProxy)
    local EquipProxy = global.Facade:retrieveProxy(global.ProxyTable.Equip)
    local equipDataByPos = TradingBankLookPlayerProxy:GetLookPlayerItemPosData_Hero()
    for pos, data in pairs(equipDataByPos) do
        local equipPanel = HeroSuperEquip_Look_TradingBank._ui["Panel_pos" .. pos]
        if equipPanel then
            local itemNode = HeroSuperEquip_Look_TradingBank._ui["Node_" .. pos]
            if itemNode then
                itemNode:removeAllChildren()
            end
            if EquipProxy:IsShowEquipItems(pos) then
                local item = TradingBankLookPlayerProxy:GetLookPlayerItemDataByMakeIndex_Hero(data.MakeIndex)
                if item then
...
function HeroSuperEquipLayer:UpdatePlayerView(noEquipType, init)
    local modelNode = HeroSuperEquip_Look_TradingBank._ui.Node_playerModel
    modelNode:removeAllChildren()

    local showNakedMold = HeroSuperEquip_Look_TradingBank._show_naked_mold ~= 1
    local TradingBankLookPlayerProxy = global.Facade:retrieveProxy(global.ProxyTable.TradingBankLookPlayerProxy)
    local EquipProxy = global.Facade:retrieveProxy(global.ProxyTable.Equip)
    local equipDataByPos = TradingBankLookPlayerProxy:GetLookPlayerItemPosData_Hero()
    ...
    local function GetLooks(equipType, need)
        ...
        if not need and equipType and equipDataByPos[equipType] then
            ...
            local equipData = TradingBankLookPlayerProxy:GetLookPlayerItemDataByMakeIndex_Hero(MakeIndex)
    ...
    local hairID = HeroSuperEquip_Look_TradingBank.playerHairID
    ...
    local sex = HeroSuperEquip_Look_TradingBank.playerSex
    local job = HeroSuperEquip_Look_TradingBank.playerJob
    local UiModel = CreateStaticUIModel(sex, modelData, nil, {job = job})
    UiModel = HeroSuperEquip_Look_TradingBank.CreateModelCallBack(UiModel, modelData)
    if UiModel then
...
function HeroSuperEquipLayer:CreateEquipItem(data)
    ...
    return HeroSuperEquip_Look_TradingBank.CreateEquipItemCallBack(goodItem, info)
end
...
function HeroSuperEquipLayer:UpdateEquipSettingShowSUI()
    HeroSuperEquip_Look_TradingBank._ui.Text_shizhuang:setVisible(false)
    HeroSuperEquip_Look_TradingBank._ui.CheckBox_shizhuang:setVisible(false)
end
