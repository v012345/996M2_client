TitleTips = {}--角色 英雄 面板 称号提示
TitleTips._ui = nil
local tipsWidth = 270
local textSize = 18

function TitleTips.main(data)
    local parent = GUI:Attach_Parent()
    TitleTips._parent = parent
    TitleTips.isWinPlayMode = SL:GetMetaValue("WINPLAYMODE")
    if TitleTips.isWinPlayMode then
        textSize = 16
    end
...
function TitleTips.InitLayer(data)
...
function TitleTips.CreateItemPanel( data )
    ...
    local strSize = TitleTips.isWinPlayMode and 12 or 16
    ...
    strSize = TitleTips.isWinPlayMode and 12 or 16
    strColor = "#ffffff"
    local richText = GUI:RichText_Create(listView, "rich_desc", 0, 0, desc, width, strSize, strColor) 
    GUI:setAnchorPoint(richText, 0, 0)
